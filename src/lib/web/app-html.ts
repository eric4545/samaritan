...
const CLIENT_JS = [
  ...
  '',
  '  // POST JSON, replace the session with the response, re-render, alert on failure.',
  '  function postAndRefresh(path, body, errLabel) {',
  '    return api(path, {',
  '      method: "POST",',
  ...
  '      renderRunView();',
  '    }).catch(function (err) { alert(errLabel + ": " + err.message); });',
  '  }',
  '',
  '  function runStepPath(index, suffix) {',
  '    return "/api/runs/" + state.session.id + "/steps/" + index + (suffix || "");',
  '  }',
  '',
  ...
  '    postAndRefresh("/api/runs", { environment: state.selectedEnv, operator: operator || undefined }, "Failed to start run");',
  '  }',
  '',
  '  function setStepStatus(index, status) {',
  '    if (!state.session) { alert("Start a run first"); return; }',
  '    postAndRefresh(runStepPath(index), { status: status }, "Failed to update step");',
  '  }',
  '',
  '  function addNote(index, note) {',
  '    if (!state.session) { alert("Start a run first"); return; }',
  '    if (!note) return;',
  '    postAndRefresh(runStepPath(index), { note: note }, "Failed to add note");',
  '  }',
  '',
  '  function addPastedEvidence(index, type, description, content) {',
  '    if (!state.session) { alert("Start a run first"); return; }',
  '    if (!content) return;',
  '    postAndRefresh(runStepPath(index, "/evidence"), { type: type, description: description || undefined, content: content }, "Failed to add evidence");',
  '  }',
  '',
  ...
  '      return postAndRefresh(runStepPath(index, "/evidence"), { type: type, description: description || undefined, filename: file.name, dataBase64: dataBase64 }, "Failed to upload evidence");',
  ...
  '      noteInput.value = "";',
  '    } else if (action === "add-evidence-text" || action === "add-evidence-file") {',
  '      var typeSel = card.querySelector(".evidence-type");',
  '      var descInput = card.querySelector(".evidence-description");',
  '      var evType = typeSel.value;',
  '      var evDesc = descInput.value.trim();',
  '      if (action === "add-evidence-text") {',
  '        var contentArea = card.querySelector(".evidence-content");',
  '        addPastedEvidence(index, evType, evDesc, contentArea.value);',
  '        contentArea.value = "";',
  '      } else {',
  '        var fileInput = card.querySelector(".evidence-file");',
  '        addFileEvidence(index, evType, evDesc, fileInput.files[0]);',
  '      }',
  '    }',
  '  });',
  '',
