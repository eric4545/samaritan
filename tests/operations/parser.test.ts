...
describe('Enhanced Operation Parser', () => {
  ...
  it('should apply include and exclude filters to matrix expansion', async () => {
    const operation = await parseFixture('matrixWithFilters');
...
`,
  ...
  describe('expect field validation', () => {
    it('throws a clear error with did-you-mean hint for unknown expect field in template', async () => {
      const { writeFileSync, unlinkSync } = await import('node:fs');
      const { join } = await import('node:path');
      const tplPath = join('/tmp', 'tpl-bad-expect.yaml');
      const opPath = join('/tmp', 'op-bad-expect.yaml');
      writeFileSync(
        tplPath,
        `- name: check\n  command: aws sts\n  expect:\n    contain: \${AWS_ACC}\n`,
      );
      ...
      try {
        await assert.rejects(
          ...
          (err: any) => {
            ...
            assert.ok(
              allMessages.includes('contain'),
              `Expected error mentioning 'contain', got: ${allMessages}`,
            );
            assert.ok(
              allMessages.includes('contains'),
              `Expected did-you-mean hint for 'contains', got: ${allMessages}`,
            );
            return true;
          },
        );
    ...
    it('accepts all known expect fields without error', async () => {
      const { writeFileSync, unlinkSync } = await import('node:fs');
      const { join } = await import('node:path');
      const opPath = join('/tmp', 'op-good-expect.yaml');
      writeFileSync(
        opPath,
        [
          'name: T',
      ...
      try {
        const op = await parseOperation(opPath);
        assert.deepStrictEqual(op.steps[0].expect, {
          contains: 'hi',
          not_empty: true,
        });
      } finally {
