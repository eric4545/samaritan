...
`
    ...
    try {
      ...
      assert.strictEqual(operation.environments[0].variables.HOST, 'example.com')
...
`
    ...
    try {
      ...
      assert.deepStrictEqual(operation.tags, ['deployment', 'production', 'automated'])
    } finally {
      fs.unlinkSync(tempFile)
    }
  })
...
`
    ...
    try {
      ...
      const production = operation.environments.find(e => e.name === 'production')
...
`
    ...
    try {
      ...
      const preflightSteps = operation.steps.filter(s => s.phase === 'preflight')
      ...
      const deploymentSteps = operation.steps.filter(s => s.phase !== 'preflight')
...
`
    ...
    try {
      ...
      const migrateStep = operation.steps.find(s => s.name === 'Migrate Database')
      assert.ok(migrateStep)
      assert.strictEqual(migrateStep.evidence?.required, true)
      assert.deepStrictEqual(migrateStep.evidence?.types, ['screenshot', 'log'])
      assert.strictEqual(migrateStep.retry?.max_attempts, 5)
      assert.strictEqual(migrateStep.retry?.backoff, 'linear')

      const deployStep = operation.steps.find(s => s.name === 'Deploy Application')
      ...
      const verifyStep = operation.steps.find(s => s.name === 'Verify Health')
...
`
    ...
    try {
      ...
      assert.strictEqual(rollback1.command, 'kubectl rollout undo deployment/database')
      ...
      assert.ok(rollback2.instruction?.includes('Rollback to previous version'))

      // Verify third rollback
      const rollback3 = operation.steps[2].rollback
      assert.ok(rollback3)
      assert.strictEqual(rollback3.timeout, 120)
      assert.strictEqual(rollback3.command, 'kubectl rollout undo deployment/worker')
    } finally {
      fs.unlinkSync(tempFile)
    }
  })
