...
function makeJsonl(events: object[]): string {
  return events
    .map((e) => JSON.stringify({ ts: new Date().toISOString(), session_id: 'f3a9b2', ...e }))
    .join('\n') + '\n';
}

const sampleJsonl = makeJsonl([
  { type: 'session_start', op: 'deployment.yaml', tmux_session: 'samaritan-f3a9b2' },
  { type: 'step_start', step: 0, name: 'Pre-flight checks', pic: 'ops@example.com' },
  { type: 'command_sent', session: 'execution', command: 'kubectl get nodes' },
  { type: 'pane_captured', session: 'execution', output: 'node/prod-1  Ready  4d\nnode/prod-2  Ready  4d' },
  { type: 'user_input', action: 'verify_ok', step: 0, actor: 'ops@example.com' },
  { type: 'step_complete', step: 0 },
  { type: 'step_start', step: 1, name: 'Deploy App', pic: 'ops@example.com', reviewer: 'sre@example.com' },
  { type: 'command_sent', session: 'execution', command: 'kubectl apply -f deployment.yaml' },
  { type: 'pane_captured', session: 'execution', output: 'deployment.apps/web created' },
...
describe('generateReport (issue #10)', () => {
  ...
  it('includes session ID and status in report', () => {
    ...
    try {
      ...
      assert.ok(md.includes('completed') || md.includes('Completed'), 'should include status');
    } finally {
      if (existsSync(jsonlPath)) unlinkSync(jsonlPath);
    }
  });
  ...
  it('includes rollback section (even if empty)', () => {
    ...
    try {
      const md = generateReport(jsonlPath);
      assert.ok(md.includes('Rollback') || md.includes('rollback'), 'should have rollback section');
    } finally {
      if (existsSync(jsonlPath)) unlinkSync(jsonlPath);
    }
  });
  ...
  it('includes rollback events in dedicated section', () => {
    const withRollback = makeJsonl([
      { type: 'session_start', op: 'deploy.yaml', tmux_session: 'samaritan-rb1' },
      { type: 'step_start', step: 0, name: 'Deploy' },
      { type: 'rollback_start', step: 0, triggered_by: 'user_input' },
      { type: 'command_sent', session: 'execution', command: 'kubectl rollout undo', context: 'rollback' },
    ...
    try {
      const md = generateReport(jsonlPath);
      assert.ok(md.includes('kubectl rollout undo') || md.includes('Rollback'), 'should show rollback event');
    } finally {
      if (existsSync(jsonlPath)) unlinkSync(jsonlPath);
    }
  });
