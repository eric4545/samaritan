...
describe('ADF Generator', () => {
  ...
  it('should generate valid ADF structure', () => {
    ...
    assert.ok(Array.isArray((adf as any).content), 'Content should be an array')
  })
  ...
  it('should resolve variables when resolveVariables is true', () => {
    const adfString = generateADFString(mockOperation, undefined, undefined, true)
    ...
    const hasResolvedVar = adfString.includes('--replicas=2') || adfString.includes('--replicas=5')
  ...
  it('should include step metadata (PIC, timeline, ticket)', () => {
    ...
    // Check for timeline
    assert.ok(adfString.includes('2024-01-15 10:00'), 'Should include timeline')

    // Check for ticket
    assert.ok(adfString.includes('JIRA-123'), 'Should include ticket reference')
  })
  ...
  it('should handle operations with dependencies', () => {
    ...
    const bulletLists = content.filter((node: any) => node.type === 'bulletList')
