...
describe('Format Consistency Tests', () => {
  it('should include operation name and version in all formats', () => {
    ...
    assert(
      markdown.includes('Deploy Web Server'),
      'Markdown should include operation name',
    );
    assert(markdown.includes('v1.1.0'), 'Markdown should include version');

    // ADF should have title
    assert(
      adfString.includes('Deploy Web Server'),
      'ADF should include operation name',
    );
  ...
  it('should include all environments in all formats', () => {
    ...
    assert(
      markdown.includes('staging'),
      'Markdown should include staging environment',
    );
    assert(
      markdown.includes('production'),
      'Markdown should include production environment',
    );

    assert(
      adfString.includes('staging'),
      'ADF should include staging environment',
    );
    assert(
      adfString.includes('production'),
      'ADF should include production environment',
    );
  });
  ...
  it('should include all steps in all formats', () => {
    ...
    stepNames.forEach((stepName) => {
      assert(
        markdown.includes(stepName),
        `Markdown should include step: ${stepName}`,
      );
      assert(
        adfString.includes(stepName),
        `ADF should include step: ${stepName}`,
      );
    });
  });
  ...
  it('should resolve variables consistently across formats', () => {
    ...
    const adfStringResolved = generateADFString(
      deploymentOperation,
      undefined,
      undefined,
      true,
    ...
    assert(
      markdownResolved.includes('--replicas=2'),
      'Markdown should resolve staging REPLICAS',
    );
    assert(
      markdownResolved.includes('--replicas=5'),
      'Markdown should resolve production REPLICAS',
    );

    assert(
      adfStringResolved.includes('--replicas=2'),
      'ADF should resolve staging REPLICAS',
    );
    assert(
      adfStringResolved.includes('--replicas=5'),
      'ADF should resolve production REPLICAS',
    );

    // Both should resolve DB_HOST variable
    assert(
      markdownResolved.includes('staging-db.example.com'),
      'Markdown should resolve staging DB_HOST',
    );
    assert(
      markdownResolved.includes('prod-db.example.com'),
      'Markdown should resolve production DB_HOST',
    );

    assert(
      adfStringResolved.includes('staging-db.example.com'),
      'ADF should resolve staging DB_HOST',
    );
    assert(
      adfStringResolved.includes('prod-db.example.com'),
      'ADF should resolve production DB_HOST',
    );
  });
  ...
  it('should use continuous step numbering across formats', () => {
    ...
    assert(
      ...
      'Markdown should have Pre-Flight phase',
    );
    assert(
      markdown.includes('Flight Phase'),
      'Markdown should have Flight phase',
    );
    assert(
      markdown.includes('Post-Flight Phase'),
      'Markdown should have Post-Flight phase',
    );

    assert(
      adfString.includes('Pre-Flight'),
      'ADF should have Pre-Flight phase',
    );
    assert(adfString.includes('Flight Phase'), 'ADF should have Flight phase');
    assert(
      adfString.includes('Post-Flight'),
      'ADF should have Post-Flight phase',
    );
  });
  ...
  it('should include metadata when provided', () => {
    ...
    assert(
      adfString.includes('staging'),
      'ADF should include target environment',
    );
  });
  ...
  it('should filter to single environment consistently', () => {
    const markdownStaging = generateManual({
      ...deploymentOperation,
      environments: deploymentOperation.environments.filter(
        (e) => e.name === 'staging',
      ),
      variables: { staging: deploymentOperation.variables.staging },
    });
    const adfStringStaging = generateADFString(
      deploymentOperation,
      undefined,
      'staging',
    );
    ...
    assert(
      markdownStaging.includes('staging'),
      'Markdown should include staging',
    );
    ...
    assert(
      !markdownStaging.match(/\| Step \| staging \| production \|/),
      'Markdown should not have production column when filtered',
    );
    ...
    const _adfObj = JSON.parse(adfStringStaging);
    const hasProductionInTable =
      adfStringStaging.includes('"production"') &&
      adfStringStaging.includes('"Live production environment"');
    assert(
      !hasProductionInTable,
      'ADF should not include production environment row when filtered',
    );
  });
  ...
  it('should include rollback procedures consistently', () => {
    ...
    assert(
      markdown.includes('Rollback'),
      'Markdown should have rollback section',
    );
    assert(
      markdown.includes('kubectl rollout undo'),
      'Markdown should have rollback command',
    );
  ...
  it('should include step metadata (PIC, timeline, ticket) consistently', () => {
    ...
    assert(
      markdown.includes('2024-01-15 10:00'),
      'Markdown should include timeline',
    );
    assert(
      adfString.includes('2024-01-15 10:00'),
      'ADF should include timeline',
    );
