...
describe('Manual Generation with Metadata Integration', () => {
  ...
  it('should generate manual with YAML frontmatter when metadata provided', async () => {
    ...
    assert(
      manual.includes('---\n\n# Manual for:'),
      'Should have frontmatter delimiter followed by content',
    );
    ...
    assert(
      manual.includes('production'),
      'Should include production environment',
    );
    assert(
      !manual.includes('staging'),
      'Should not include staging when filtered to production',
    );
  });
  ...
  it('should generate manual without frontmatter when no metadata provided', () => {
    ...
    assert(
      manual.startsWith('# Manual for:'),
      'Should start directly with manual content',
    );
  ...
  it('should handle metadata with no target environment', async () => {
    ...
    assert(
      !manual.includes('target_environment:'),
      'Should not include target_environment when not specified',
    );
    ...
    assert(
      manual.includes('| Step | staging | production |'),
      'Should include both environment columns',
    );

    // Should resolve variables since resolveVariables=true
    assert(manual.includes('--replicas=2'), 'Should resolve staging variables');
    assert(
      manual.includes('--replicas=5'),
      'Should resolve production variables',
    );
  });
  ...
  it('should combine environment filtering, variable resolution, and metadata', async () => {
    ...
    assert(
      manual.includes('source_file: "operations/advanced-deployment.yaml"'),
    );
    ...
    assert(
      manual.includes('| Step | staging |'),
      'Should have only staging column',
    );
    assert(
      !manual.includes('| production |'),
      'Should not include production column when filtered',
    );

    // Should resolve variables for staging
    assert(
      manual.includes('--replicas=2'),
      'Should resolve staging REPLICAS variable',
    );
    assert(
      !manual.includes('--replicas=5'),
      'Should not include production values when filtered to staging',
    );
    assert(
      !manual.includes('${REPLICAS}'),
    ...
    assert(
      !manual.includes('| production |'),
      'Should not contain production environment row when filtered',
    );
  });
