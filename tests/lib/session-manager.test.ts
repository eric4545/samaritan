...
import type { Operation } from '../../src/models/operation.js';
...
test('SessionManager - listSessions and filtering', () => {
  ...
  const _session3 = sessionManager.createSession('op-1', 'prod', 'operator3');
...
test('SessionManager - SessionUtils.formatSessionDuration', () => {
  ...
  session.started_at = new Date(
    now.getTime() - (2 * 60 * 60 * 1000 + 30 * 60 * 1000 + 45 * 1000),
  );
