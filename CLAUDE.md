...
- **Operation inheritance** (`extends:`, `src/operations/extends.ts`) — a child operation inherits environments/variables/steps/rollback from one or more base operation files (string or array, merged left→right, child wins last); scalars are last-wins, `variables`/`common_variables` spread-merge, `environments` concatenate (same-name entries merge via the existing `uses:` logic), `steps` **append** (`base ++ child`, no override-by-id), `rollback` is whole-object last-wins. A base's relative paths (`script`, `evidence.results[].file`, step `uses:`, `env_file`, `environments[].uses`) are rebased to absolute against that base's own directory before merging; only non-root files (bases) are rebased — the file actually passed to `parseOperation` keeps its authored paths untouched. Circular `extends:` chains throw `Circular extends detected: <chain>`; remote bases (`github:`/`https:`) are rejected in this phase.
