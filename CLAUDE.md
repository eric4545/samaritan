...
approved) until at least one item is captured or the operator overrides with a
logged reason. `types`/`results` above remain purely documentation/embedding —
only `required` gates completion, independent of which types are declared.
...
- `samaritan run` - Interactive loop is implemented. **Default mode is now `sidecar`** (since v1.1). In sidecar mode, samaritan DISPLAYS each step's resolved command but does NOT send it to tmux — the operator runs it themselves, then presses `[v]` to verify `step.expect`. Both spawn-own tmux (`sessions:` in YAML) and attach-to-existing pane (`--attach <tmux-target>`) are supported, plus mid-flight attach/switch via `[t]`. See `CaptureBackend` in `src/lib/capture-backend.ts` and `TmuxPaneCapture` in `src/lib/tmux-session.ts`.
  ...
  **Evidence-required gate (v1.1+)**: `needsEvidenceGate(i, step)`/`confirmEvidenceGate(i, captureSinceStepStart?)` in `run.ts` gate completing a step whose `evidence.required` is true and has zero captured items (`stepEvidence(i).length === 0`). Inserted at TWO sites: the manual/sidecar completion path (right before `executor.executeStepManually` in the `manual`/sidecar branch) and the `approval` step's `choice === 'approve'` branch (before `recordApprovalDecision`) — `[s]`kip/`abort`/`[b]`ack/`[j]`ump/rollback are checked earlier in each branch and unaffected. `confirmEvidenceGate` offers `[e]` (reuses the existing `captureEvidence` flow; loops back if still empty) or `[o]` override — override logs a `user_input`/`action:'override'` event with a `reason` field, structurally identical to `promptAssertFailureAction`'s own override branch (same event shape, so both render identically in the audit log/report). Declining (Enter) returns `false`; the caller does `i--; continue;` to re-enter the SAME step next loop iteration (same idiom as `[b]`/`[j]`'s `i = navTarget - 1; continue`), so the step re-displays instead of advancing. Enforcement is a per-run boolean (`requireEvidence`, threaded as the 9th param to `runInteractiveStepLoop`), computed as `options.requireEvidence !== false` in both `runOperation` and `resumeSession` — ON by default; `--no-require-evidence` (Commander negatable option, present on both `run` and `resume`) is the only way to disable it. Does NOT gate plain (non-sidecar) `type: automatic` steps — only `manual`/sidecar-automatic completion and `approval`'s `approve` path. Fixture `tests/fixtures/operations/features/evidence-required.yaml`; tests in the `evidence-required gate` describe block of `tests/cli/run.test.ts` — note the multi-question flows there (gate → `o`/`e` sub-prompts) hit the readline "freshly-invoked async helper" gotcha described below under manual-step testing notes, so they use the sleep-delimited `bash -c` piping technique (`runSequenced` helper) instead of a single chained `input:` string. Example `examples/evidence-required-step.yaml`.
