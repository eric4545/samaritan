...
- `samaritan run` - Interactive loop is implemented. **Default mode is now `sidecar`** (since v1.1). In sidecar mode, samaritan DISPLAYS each step's resolved command but does NOT send it to tmux — the operator runs it themselves, then presses `[v]` to verify `step.expect`. Both spawn-own tmux (`sessions:` in YAML) and attach-to-existing pane (`--attach <tmux-target>`) are supported, plus mid-flight attach/switch via `[t]`. See `CaptureBackend` in `src/lib/capture-backend.ts` and `TmuxPaneCapture` in `src/lib/tmux-session.ts`.
  ...
  **Interactive prompt for missing run-time variables**: `--var KEY=VALUE` (repeatable) has always let an operator supply a variable's value only at `run` time — e.g. reusing one operation manual across dates via `--var REPORT_DATE=2026-07-14`, with no default anywhere in the YAML. Since v1.1, if a `${VAR}` still has no value after merging `common_variables` → `operation.variables[env]` → `--var` and `run`/sidecar is attached to a real terminal (`process.stdin.isTTY`), `OperationRunner.promptForMissingVariables` (`src/cli/commands/run.ts`) prompts for each missing name once, up front, before the session/executor are created — scanned via the pure helper `collectUnresolvedVars` (`src/lib/variable-resolver.ts`, a `JSON.stringify` + `listUnresolvedVars` scan over `execOperation.steps`/`rollback` so every `StepContent` field, including nested `sub_steps` and rollback, is covered without hand-enumerating fields). Answers mutate the resolved vars object in place, so they flow into `context.variables`, the persisted session (and therefore `resume`), and `report.md` with no other call site changes. Blank answers are left unresolved on purpose, falling through to the pre-existing `tryResolve` warn-and-continue path (`"⚠️  Unresolved variable(s): ..."`). Gated on TTY rather than "interactive mode" alone — an unattended/CI run that forgot `--var` must never hang on a prompt nobody can answer, so `--auto-approve`, `--dry-run`, and any non-TTY invocation always skip straight to the warning. `--no-prompt` opts out even on a real TTY. Because `spawnSync`'s piped `input` is never a TTY, this prompt (like `[t]` attach / paste-buffer / Ctrl+C below) isn't integration-testable via the existing CLI test harness — `tests/cli/run.test.ts` covers the "prompt is skipped" side of the gate (regression: `--no-prompt` and non-TTY both still warn-and-continue, and never hang) plus unit tests for `collectUnresolvedVars`; the enabled path is smoke-tested manually with `examples/report-with-runtime-date.yaml`.
